%% MF14 Vehicle Parameters Recovered from HEAD
% This parameter script reproduces the baseline vehicle values assigned in
% HEAD:VehicleBalance_MF14.m. That HEAD revision explicitly warns that its
% active setup matches MF13 values, so the data is named MF13 here.
%
% The script uses the same `setup` and `steeringSetups` schema as
% VehicleBalance_MF14_parameters.m and can be sourced by the refactored
% VehicleBalance_MF14.m analysis.
%
...
setup.W_unsprungF = 38.8;      % recovered from m_uf = 38.8/32.2 [lbf]
setup.W_unsprungR = 38.2;      % recovered from m_ur = 38.2/32.2 [lbf]
...
setup.unsprung_z = setup.r_l;  % HEAD derives unsprung CG from tire radius [in]
...
%% Aerodynamics
setup.CL = 3.71;               % positive coefficient creates downforce [-]
setup.CD = 1.71;               % drag coefficient [-]
setup.DFDistF = 0.43;          % front downforce fraction [-]
setup.rhoAir = 1.225;          % air density [kg/m^3]
setup.aeroArea = 1.08;         % reference area [m^2]

%% Suspension
setup.rc_zf = 2.329;           % front roll-center height [in]
setup.rc_zr = 2.644;           % rear roll-center height [in]
...
setup.kRoll_r_arb = [0:100:400];       % effective rear ARB stiffness [N*m/deg]
