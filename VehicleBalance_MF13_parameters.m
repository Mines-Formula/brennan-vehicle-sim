%% MF13 Vehicle Parameters
% This parameter script is sourced by a balance-analysis driver. It populates
% `setup` and `steeringSetups` in the caller workspace; it does not clear
% existing variables or run the balance model.
%
% FIXME: Confirm the MF14-specific setup before treating results as final.
%
...
% Cartesian sweep in the main script.
...
setup.weightDistL = 0.51;      % static left weight fraction [-]
...
setup.KPI = 7.6;               % kingpin inclination [deg]

%% Maneuver Geometry
setup.r_corner = 10;           % centerline corner radius [m]

%% Aerodynamics
setup.CL = 3.05;               % positive coefficient creates downforce [-]
setup.CD = 3.05 / 2;           % preserves prior CL/CD = 2 baseline
setup.DFDistF = 0.46;          % front downforce fraction [-]
...
setup.kWheel_f = 307.5;        % front wheel rate [lbf/in]
setup.kWheel_r = 272.5;        % rear wheel rate [lbf/in]
setup.kRoll_f_arb = 0;         % effective front ARB stiffness [N*m/deg]
setup.kRoll_r_arb = 317.6;     % effective rear ARB stiffness [N*m/deg]
